...
//		Helper::setServiceManagaer($e->getApplication()->getServiceManager());
        ...
        $eventManager->getSharedManager()->attach('Zend\Mvc\Application', 'dispatch', function ($e) use ($serviceManager) {
    ...
    public function dispatchError(MvcEvent $e)
    {
        ...
        {
            $logger->err($exception->getMessage(). " - ErrorCode: ".$exception->getCode(). " File: ". $exception->getFile(). " Line: ". $exception->getLine());
        }
        else {
            $logger->err($error);
        }
